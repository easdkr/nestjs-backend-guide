...
export class Migration20251004044905 extends Migration {
  override async up(): Promise<void> {
    ...
    this.addSql(
      `alter table "product" 
      add constraint "product_store_id_foreign"
      foreign key ("store_id") references "store" ("id") on update cascade;`,
    );

    this.addSql(
      `alter table "option_group" 
      add constraint "option_group_product_id_foreign"
      foreign key ("product_id") references "product" ("id") on update cascade;`,
    );

    this.addSql(
      `alter table "option" 
      add constraint "option_option_group_id_foreign"
      foreign key ("option_group_id") references "option_group" ("id") on update cascade;`,
    );

    this.addSql(
      `alter table "option_price" 
      add constraint "option_price_option_id_foreign"
